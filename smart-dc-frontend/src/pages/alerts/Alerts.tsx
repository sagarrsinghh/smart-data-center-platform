...
export default function Alerts() {
  ...
  const [underutilizedAssets, setUnderutilizedAssets] = useState<any[]>([]);
  ...
  const fetchAlerts = async () => {
    try {
      ...
      if (payload) {
         ...
         setUnderutilizedAssets(payload.underutilizedAssets || []);
      }
    ...
    <MainLayout>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
            ...
            >
              <div className="px-6 py-4 border-b border-white/[0.06]">
                <h2 className="text-base font-semibold text-white">Underutilized / Idle Storage Assets</h2>
                <p className="text-xs text-gray-500 mt-1">Devices with &gt;= 5 TB allocated capacity but under 15% physical utilization.</p>
              </div>
              
              {underutilizedAssets.length === 0 ? (
                <div className="py-8 text-center text-emerald-400/80 text-sm">No underutilized storage assets detected.</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-white/[0.02] border-b border-white/[0.06]">
                        <th className="px-6 py-4 text-left text-xs uppercase tracking-wider text-gray-400">Device Name</th>
                        <th className="px-6 py-4 text-left text-xs uppercase tracking-wider text-gray-400">Location</th>
                        <th className="px-6 py-4 text-left text-xs uppercase tracking-wider text-gray-400">Allocated (TB)</th>
                        <th className="px-6 py-4 text-left text-xs uppercase tracking-wider text-gray-400">Used (TB)</th>
                        <th className="px-6 py-4 text-left text-xs uppercase tracking-wider text-gray-400">Wasted (TB)</th>
                        <th className="px-6 py-4 text-left text-xs uppercase tracking-wider text-gray-400">Utilization</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/[0.04]">
                      {underutilizedAssets.map((asset) => (
                        <tr key={asset.deviceName} className="hover:bg-white/[0.02]">
                          <td className="px-6 py-4 text-white font-medium">{asset.deviceName}</td>
                          <td className="px-6 py-4 text-gray-400">{asset.locationCode}</td>
                          <td className="px-6 py-4 text-gray-300">{asset.allocatedCapacityTb}</td>
                          <td className="px-6 py-4 text-gray-300">{asset.usedCapacityTb}</td>
                          <td className="px-6 py-4 text-amber-500 font-medium">{asset.wastedCapacityTb}</td>
                          <td className="px-6 py-4">
                            <span className="text-yellow-400 font-semibold">{asset.utilizationPercent}%</span>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-white/[0.02] border-b border-white/[0.06]">
                        ...
                        <th className="px-6 py-4 text-left text-xs uppercase tracking-wider text-gray-400">Total VMs</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/[0.04]">
                      {highComputeProjects.map((project) => (
                        <tr key={project.projectName} className="hover:bg-white/[0.02]">
                          ...
                          <td className="px-6 py-4 text-gray-300">{project.totalVms}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
